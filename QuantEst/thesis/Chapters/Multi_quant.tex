...
In the experiment of Fig \ref{fig: shiftQ_SGD}, $0$ crossing occurs in the shiftQ implementation, while 33 crossings happened over 5000 SGD estimation epochs. Over 100 experiments on the same data distribution over SGD, the average crossing number is 72.48, and the standard deviation is 66.23. The crossing in SGD has two interesting properties:
    \begin{enumerate}
...
Suppose the straightforward quantile computation for [$\tau_1, ..., \tau_K$] that sorts all observations from the dataset $X = \{x_i\}^N_1$. Let $x_i$ denote the $i$th smallest value of $X$, then we have $x_1 < x_2 < ... < x_N$. 
...
When the parabolic assumption cannot be applied to the quantile value update, the alternative update is a \textit{piecewise-linear formula}:
\begin{equation}
    \begin{array}{l}
...
It means, in the initialization part, there will be $2K+3$ markers for
$$
...
The extended $P^2$ algorithm is an extended version of the $P^2$ algorithm, so the algorithm \ref{alg:multi_p2} of $P^2$ is shown before the algorithm  \ref{alg:multi_ext_p2} of the extended $P^2$.
\begin{algorithm}
    \caption{The $P^2$ Algorithm}\label{alg:multi_p2}
        \begin{algorithmic}[1]
            \Require{Dataset $X$, $K$ Demanding quantile values $[\tau_1, \tau_2, ..., \tau_K]$}
            \Ensure{Demaning quantile estimates $[\tau_1\text{-}q, \tau_2\text{-}q, ..., \tau_K\text{-}q]$}
            \State
            \State{\textbf{A. Initialization}}
            \State {The first $K$ observations (sorted): \{$x_1,x_2,...,x_K$\}}
            \For{$(i = 1, ..., K-1)$}
            ...
            \State{$m_i = m_i + 1$; $i = k+1, ..., K$}       \Comment{Increment some marker positions}
            ...
            \State {$[\tau_1\text{-}q, \tau_2\text{-}q, ..., \tau_K\text{-}q] = [q_1, q_2, ..., q_K]$}
        \end{algorithmic}
\end{algorithm}
\\\\
...
\begin{algorithm}
    \caption{Extended $P^2$ Algorithm}\label{alg:multi_ext_p2}
        \begin{algorithmic}[1]
            ...
            \State{$[\tau_1\prime, \tau_2\prime, ..., \tau_{2K+3} \prime] = [0, \frac{0+\tau_1}{2}, \tau_1, \frac{\tau_1 + \tau_2}{2}, \tau_2, ..., \tau_{M}, \frac{\tau_K+1}{2}, 1]$}
