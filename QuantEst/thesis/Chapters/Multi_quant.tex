    ...
    \begin{algorithmic}[1]
        ...
        \State {Initialize} $q$                 \Comment{Requires \textbf{positive initialization} $q$ > 0}
            \For{$x_k$ in $X$}                  \Comment{Parameter update for each input data point}
                ...
                \If{$x_k > q$}                  
                    \State{$q = q + \alpha_k \tau \cdot q$}
                \Else                           
                    \State{$q = q - \alpha_k (1-\tau)\cdot q$}
                \EndIf
            \EndFor
        \State \textbf{return} $q$              \Comment{$q$ is the DUMIQE result of quantile estimate}
    \end{algorithmic}
\end{algorithm}
...
%     \item Shift the change back to $X$: $Q_x(q_{k+1}) = Q_y(q_{k+1}) + \delta$
% \end{itemize}
