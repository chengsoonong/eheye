...
%             \State
%             \State {\textbf{Extend $N$ quantile values to $m$ quantile values}}
%             \LineComment{Evenly fill the intervals between $0,1$ and each 2 quantile values}
%             \State {$m = 2N + 3$}
%             \State {\text{  } $\tau_1^{new}, tau_2^{new},tau_3^{new},tau_4^{new},$\hspace{18pt}$ ...$\hspace{6pt}$, tau_{m-1}^{new}, $\hspace{12pt}$tau_{m}^{new}$}
%             \State {$ = 0, $ \hspace{12pt}$\tau_1/2, $ \hspace{15pt}$\tau_1, $\hspace{26pt}$(\tau_1 + \tau_2)/2, $\hspace{6pt}$...$\hspace{6pt}$, (\tau_M + 1)/2, 1$}
            
%             \State
%             \State{\textbf{Feed the extended quantile values to a $P^2$ algorithm}}
%             \State{extended quantile values = $[\tau_{1}^{new},\tau_{2}^{new},..., \tau_{m-1}^{new},\tau_{m}^{new} ]$}
%             \State{$[q_0, q_1, ..., q_m] = P^2(\text{dataset X, )$}

%             \State {Extract the quantile estimates for the original $N$ quantile values}
%             \State {$[\tau_1\text{-}q, \tau_2\text{-}q, ..., \tau_M\text{-}q] = [q_1, q_2, ..., q_M]$}
%         \end{algorithmic}
% \end{algorithm}
